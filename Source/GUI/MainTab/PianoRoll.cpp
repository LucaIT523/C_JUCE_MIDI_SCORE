...
{   
    g.setColour(Colour(AppData::componentMusicScoreBackgroundColour));
    ...
    midiHelper(midiHelperIn)
{
    setOpaque(false);
    reset();
    //GuidoViewer* gmnview = new GuidoViewer();              //////////////////////////////////////////////
    //getWindowHandle()->setContentOwned(gmnview, false);                       ///////////////////////////////////////////////////////
    //setVisible(true);                                      /////////////////////////////////////

    //	Font::setFallbackFontName (Font::getDefaultSansSerifFontName());
    
    m_guido.GuidoInit(10000, 300, 0, 0);                            ///////////////////////////////
    //gmnview->LoadFile();                                    ///////////////////////////////////////////////
    m_guido.setGMNFile("D:\\sample.gmn");
...
*/
    ...
    if (markerLineX != -1)
    {
        ...
        g.setColour(Colour(AppData::defaultLineColour));
...
{
    xOffset = 120;//getParentComponent()->getWidth() / 3;
...
void PianoRoll::newFileLoaded()
{
    reset();
    MidiSequence& midiData(midiHelper.getMidiPlayer()->getMidiSequence());
    double currentMidiTime = 0; int i = 0;
    while(currentMidiTime <= midiData.getEndTime())
    {
        i++;
...
int PianoRoll::getXOffsetFromMidiTime(double time)
{
    ...
    MidiSequence& midiData(midiHelper.getMidiPlayer()->getMidiSequence());
    Array<double> temp =midiData.getNextNoteOnTimes(time);
    double sequenceStartTime = midiData.getStartTime();
    double sequenceEndTime = midiData.getEndTime();
    double pixelPerTime = (double)(getWidth() - xOffset) / (sequenceEndTime - sequenceStartTime);

    offset = (int) (time * pixelPerTime) + xOffset;
    int currentIndex = m_guido.getCurrentIndex();
    offset = (int) (m_guido.getCurrentPos(currentIndex)/9)+50;
    m_guido.getNextIndex();
