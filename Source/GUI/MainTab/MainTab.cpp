...
#include "../Source/gmn/include/fermata.h"
#include "../Source/gmn/fermata/funcs.h"
#include "../Source/gmn/fermata/import.h"
      ...
      pianoRollZoom(pianoRoll, pianoRollViewport)      
{
    ...
    addAndMakeVisible(&pianoRollViewport);//midi notes
...
bool MainTab::loadMidi()
{
    ...
    else
    {
        ...
        char Mode; // SILENT || VERBOSE

        char*   param[2];
        char	MidiPath[256];
        char*   GmmPath;
        strcpy_s(MidiPath, myChooser.getResult().getFullPathName().toRawUTF8());

        param[0] = MidiPath;
        param[1] = MidiPath;

        std::filesystem::path current_path = std::filesystem::current_path();
        current_path += "\\temp.gmn";
        GmmPath = (char *)current_path.c_str();
        //current_path.append("\\temp.gmn");

        readMidiFile(2, param, &Mode, GmmPath);
    }
